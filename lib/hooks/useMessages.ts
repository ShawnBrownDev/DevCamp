...
export function useMessages(channelId: string | null) {
  ...
  useEffect(() => {
    ...
    const subscription = supabase.channel(`messages:${channelId}`)
      .on('postgres_changes', 
        { 
          event: '*', 
          schema: 'public', 
        ...
        async (payload) => {
          ...
          if (payload.eventType === 'INSERT' && optimisticUpdatesRef.current.has(payload.new?.id)) {
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      setMessages(prev => prev.filter(msg => !optimisticUpdatesRef.current.has(msg.id)));
    }
  }, [channelId, supabase]);
